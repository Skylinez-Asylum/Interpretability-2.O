...
for epoch in range(config['num_epochs']):
    ...
    for batch_idx, (x, _) in enumerate(train_dataloader):
        ...
        with torch.no_grad():
            dec_norm = model.W_dec.norm(dim=1, keepdim=True)  # Honestly, the loss seems to go faster without this block, idk if this makes the loss more stable in the long run
            model.W_dec.data.div_(dec_norm)
